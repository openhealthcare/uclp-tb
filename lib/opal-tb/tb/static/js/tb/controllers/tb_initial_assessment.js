angular.module('opal.controllers').controller('TBInitialAssessmentCtrl',
  function(step, scope, episode, recordLoader, $window, Item, recordLoader) {
    ...
    recordLoader.then(function(fields){
      scope.addTest = function(){
        var item = new Item("investigation", episode, fields["investigation"]);
        episode.recordEditor.openEditItemModal(item, "investigation").then(function(result){
          if(result === 'save'){
            var copied = item.makeCopy();
            if(!copied.date_ordered){
              copied.date_ordered = new Date();
            }
            scope.editing.investigation.push(copied);
          }
        });
      };
    });
